...
public sealed class CssParser(string css) {
    public static readonly List<(Selector, Dictionary<string, string>)> DefaultCss =
        new CssParser(Resources.DefaultCss).Parse();
    ...
    private void SkipOneChar(char ch) {
        if (!(_index < css.Length && css[_index] == ch)) throw new Exception("Parsing error:" + ch);
        _index += 1;
    }
    ...
    private string ParseWord() {
        ...
        while (_index < css.Length)
            if (char.IsAsciiLetterOrDigit(css[_index]) || "#-.%".Contains(css[_index]))
                _index += 1;
            else
                break;
    ...
    public static int CascadePriority((Selector, Dictionary<string, string>) rule) {
        var (selector, _) = rule;
        return selector.Priority;
    }

    public static void ApplyCss(HtmlNode node, List<(Selector, Dictionary<string, string>)> rules) {
        ...
        foreach (var (selector, body) in rules) {
            ...
            foreach (var (property, value) in body) node.Styles[property] = value;
        }
        if (node is HtmlElement htmlElement && htmlElement.Attributes.TryGetValue("style", out var css)) {
            var pairs = new CssParser(css).ParseBody();
            foreach (var (property, value) in pairs) node.Styles[property] = value;
        }
        if (node.Styles["font-size"].EndsWith('%')) {
            var parentFontSize =
                node.Parent != null ? node.Parent.Styles["font-size"] : InheritedProperties["font-size"];
            var nodePercentage = (float)Convert.ToDouble(node.Styles["font-size"][..^1]) / 100;
            var parentPercentage = (float)Convert.ToDouble(parentFontSize[..^2]);
            node.Styles["font-size"] = nodePercentage * parentPercentage + "px";
        }
