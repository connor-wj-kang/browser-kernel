...
public sealed class BlockLayout(HtmlNode node, Layout? parent = null, Layout? previous = null)
        ...
        switch (node) {
            case HtmlText htmlText: {
                foreach (var word in htmlText.Text.Split(' ')
                             ...
                             .Select(text => text.Replace("\r", ""))) Word(node, word);
                break;
            }
    ...
    private LayoutMode GetLayoutMode() {
        ...
        if (Node.Children.Exists(child =>
                child is HtmlElement htmlElement && BlockElements.Contains(htmlElement.TagName)))
            return LayoutMode.Block;
        if (Node.Children.Count != 0 || Node is HtmlElement { TagName: "input" }) return LayoutMode.Inline;
        return LayoutMode.Block;
    }
    ...
    public override List<DrawCommand> PaintEffects(List<DrawCommand> drawCommands) {
        return Blend.PaintVisualEffects(Node, drawCommands, GetBlockLayoutRectangle());
    }
}