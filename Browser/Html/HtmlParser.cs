...
public class HtmlParser(string html) {
    ...
    private void AddImplicitElements(string tagName = "") {
        ...
        while (true) {
            var openTags = _unfinishedElements.Select(element => element.TagName).ToArray();
            ...
            else if (openTags is ["head", "html"] && tagName != "/head" && !HeadTags.Contains(tagName))
                ParseElement("/head");
            else
                break;
        }
    ...
    private void ParseElement(string element) {
        ...
        if (tagName.StartsWith('/')) {
            if (_unfinishedElements.Count == 1) return;
            ...
            var parent = _unfinishedElements.Peek();
            var htmlElement = new HtmlElement(tagName, attributes, parent);
            parent.Children.Add(htmlElement);
        }
        else {
    ...
    private static (string, Dictionary<string, string>) ParseTagNameAndAttributes(string tagContent) {
        var parts = tagContent.Split(' ');
        var tagName = parts[0].ToLower();
        var attributes = new Dictionary<string, string>();
        foreach (var attributePair in parts[1..]) {
            if (attributePair == "/" || string.IsNullOrWhiteSpace(attributePair)) continue;
            if (attributePair.Contains('=')) {
                var keyValue = attributePair.Split('=', 2);
                var key = keyValue[0].ToLower();
                var value = keyValue[1];
                if (value.Length > 2 && "'\"".Contains(value[0])) value = value[1..^1];
