...
public sealed class Blend : DrawCommand {
    ...
    public override void Draw(SKCanvas canvas) {
        var paint = new SKPaint {
            Color = SKColors.Black.WithAlpha((byte)(_opacity * 255)),
    ...
    private static SKBlendMode ParseBlendMode(string blendMode) {
    ...
    public static List<DrawCommand>
        PaintVisualEffects(HtmlNode node, List<DrawCommand> drawCommands, SKRect rectangle) {
        var opacity = (float)Convert.ToDouble(node.Styles.GetValueOrDefault("opacity", "1.0"));
        ...
        var borderRadius = (float)Convert.ToDouble(node.Styles.GetValueOrDefault("border-radius", "0px")[..^2]);
        if (blendMode == "") blendMode = "source-over";
        drawCommands.Add(new Blend(1.0f, "destination-in", [new DrawRoundRectangle(rectangle, borderRadius, "white")]));
        return [new Blend(opacity, blendMode, drawCommands)];
    }
}